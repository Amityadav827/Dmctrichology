/**
 * MIGRATION & SEEDING: MongoDB aboutdrnivedita & aboutdrniveditaleads → Supabase
 *
 * Run: node migrateDrNiveditaToSupabase.js
 *
 * What it does:
 *  1. Connects to MongoDB and fetches the singleton document from 'aboutdrnivedita' collection
 *  2. Connects to MongoDB and fetches all documents from 'aboutdrniveditaleads' collection
 *  3. Upserts page settings into Supabase 'about_dr_nivedita' with id: 1
 *  4. Inserts/upserts all lead records into Supabase 'about_dr_nivedita_leads'
...
const migrate = async () => {
  ...
  try {
    ...
    const settingsCollection = db.collection("aboutdrnivedita");
    let settingsDoc = await settingsCollection.findOne({});
    let settingsSuccess = false;

    if (!settingsDoc) {
      console.warn("⚠️ No Dr. Nivedita page settings found in MongoDB! Settings migration will be skipped.");
    } else {
      ...
      if (settingsErr) {
        ...
        if (settingsErr.message.includes("relation \"public.about_dr_nivedita\" does not exist")) {
          console.error("\n👉 IMPORTANT: You must run the SQL statements in 'backend/dr_nivedita_supabase_schema.sql' inside your Supabase SQL Editor first!");
          mongoClient.close();
          return;
        }
      } else {
    ...
    const leadsCollection = db.collection("aboutdrniveditaleads");
    ...
    if (allLeads.length > 0) {
      for (const lead of allLeads) {
        ...
        const mappedLead = {
          ...
          mobile: lead.mobile || "",
          email: lead.email || "",
          service: lead.service || "Dr. Nivedita Consultation",
    ...
    console.log("\n========================================");
    console.log(`🎉 DR. NIVEDITA MIGRATION STATUS`);
