...
import { Plus, Edit2, Trash2, Search, Filter, X, CheckCircle, ChevronDown, Check } from "lucide-react";
...
function BlogCategory() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      toast.error("Unable to load blog categories");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await updateBlogCategory(editingItem._id, form);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Delete this blog category? Ensure no blogs are linked to it.")) return;
    setActionId(id);
    try {
  ...
  if (loading) return <Loader label="Loading blog categories..." />;
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-[28px] shadow-sm border border-slate-100">
        <div>
          <h3 className="text-2xl font-bold text-slate-900">Blog Categories</h3>
          <p className="text-sm text-slate-500 mt-1">Manage dynamic categories for your blog posts</p>
        </div>
        <button
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 border-bottom border-slate-100">
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Name & Slug</th>
              ...
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item) => (
                  <tr key={item._id} className="hover:bg-slate-50 transition-colors duration-200">
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-slate-600">{item.order}</td>
                    <td className="px-6 py-4">
                      <div 
          ...
          <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h4 className="text-xl font-bold text-slate-900">
                {editingItem ? "Edit Blog Category" : "New Blog Category"}
              </h4>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="e.g. Hair Loss Treatment"
                  required
                />
              </div>
              <div className="space-y-2">
