...
function VideoCategory() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingItem) {
        ...
        toast.success("Video category updated");
      } else {
        await createVideoCategory(form);
        toast.success("Video category created");
      }
      closeModal();
      fetchItems(pagination.page, query);
    } catch (error) {
      toast.error(error.response?.data?.message || "Unable to save video category");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success("Video category deleted");
      fetchItems(pagination.page, query);
    } catch (error) {
      toast.error(error.response?.data?.message || "Unable to delete video category");
    } finally {
      setActionId("");
    }
  };
    ...
    <div className="space-y-6">
      <div className="rounded-[28px] bg-white p-5 shadow-panel">
        <div className="flex flex-col gap-4 lg lg lg">
          <div>
            <h3 className="text-2xl font-semibold text-ink">Video Category List</h3>
            <p className="mt-1 text-sm text-slate-500">
              Manage video categories with search, paging and inline sorting.
            </p>
          </div>
          <button
            type="button"
      ...
      {loading ? (
        <Spinner label="Loading video categories..." />
      ) : (
        <>
          <Table
            columns={[
      ...
      <Modal
        open={modalOpen}
        title={editingItem ? "Edit Video Category" : "Add Video Category"}
        onClose={closeModal}
        onSubmit={handleSubmit}
        submitLabel={editingItem ? "Update Video Category" : "Create Video Category"}
        loading={saving}
      >
