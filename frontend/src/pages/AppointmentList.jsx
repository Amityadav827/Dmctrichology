...
function AppointmentList() {
  ...
  const handleQuickConvert = async (id, e) => {
    ...
    const previousItems = [...items];
    setItems(prev => prev.map(item => item._id === id ? { ...item, status: "converted" } : item));

    try {
      await updateAppointment(id, { status: "converted" });
      toast.success("Lead marked as converted successfully");
    } catch (error) {
      setItems(previousItems);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await bulkDeleteAppointments({ ids: selectedIds });
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-[28px] shadow-sm border border-slate-100">
        <div>
          <h3 className="text-2xl font-bold text-slate-900">Consultation Requests</h3>
          <p className="text-sm text-slate-500 mt-1">Monitor and manage customer consultation requests</p>
        </div>
      ...
      {loading && items.length === 0 ? (
        <Loader label="Retrieving consultation requests..." />
      ) : items.length === 0 ? (
        /* Empty State Card */
        <div className="bg-white/50 backdrop-blur-md rounded-[28px] border border-slate-100 p-12 text-center max-w-xl mx-auto my-12 shadow-sm">
          ...
          <h4 className="text-xl font-bold text-gray-800 mb-2">No consultation requests yet</h4>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full mx-4 shadow-2xl border border-gray-100 transform scale-100 transition-all duration-300">
            ...
            <p className="text-gray-600 mb-6 text-sm leading-relaxed">
              This action cannot be undone. You are about to permanently delete <strong>{selectedIds.length} consultation request{selectedIds.length > 1 ? 's' : ''}</strong> from the database.
            </p>
