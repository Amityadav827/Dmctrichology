...
import {
  deleteScienceConsultation,
  bulkDeleteScienceConsultations,
  exportScienceConsultationsCsv,
  getScienceConsultations,
  updateScienceConsultation,
} from "../api/services";
...
function ScienceConsultationList() {
  ...
  const fetchItems = async (page = pagination.page) => {
    setLoading(true);
    try {
      const response = await getScienceConsultations({
        page,
      ...
      toast.error("Unable to load science consultation leads");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id, e) => {
    ...
    if (!window.confirm("Are you sure you want to delete this science consultation lead?")) return;
    setActionId(id);
    ...
    try {
      await deleteScienceConsultation(id);
      toast.success("Science lead deleted successfully");
  ...
  const handleQuickContact = async (id, e) => {
    ...
    try {
      await updateScienceConsultation(id, { status: "contacted" });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await bulkDeleteScienceConsultations({ ids: selectedIds });
      toast.success("Selected science leads deleted successfully");
      setSelectedIds([]);
      setShowModal(false);
      fetchItems(1);
    } catch (error) {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      await exportScienceConsultationsCsv({
        search,
        status,
        startDate,
        endDate
      });
      toast.success("Science leads exported successfully");
    } catch (error) {
  ...
  const handleSaveCRM = async (e) => {
    ...
    setSavingCrm(true);
    try {
      await updateScienceConsultation(crmModal.item._id, crmForm);
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-[28px] shadow-sm border border-slate-100">
        <div>
          <h3 className="text-2xl font-bold text-slate-900">Science Consultation Leads</h3>
          <p className="text-sm text-slate-500 mt-1">Monitor and manage leads from the Science at DMC page consultation form</p>
        </div>
      ...
      {loading && items.length === 0 ? (
        <Loader label="Retrieving science leads..." />
      ) : items.length === 0 ? (
        /* Empty State Card */
        <div className="bg-white/50 backdrop-blur-md rounded-[28px] border border-slate-100 p-12 text-center max-w-xl mx-auto my-12 shadow-sm">
          ...
          <h4 className="text-xl font-bold text-gray-800 mb-2">No science leads yet</h4>
          <p className="text-gray-500 text-sm leading-relaxed max-w-xs mx-auto">
            Submissions from the Science at DMC page consultation form will appear here.
          </p>
        </div>
      ) : (
        /* Table */
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full mx-4 shadow-2xl border border-gray-100 transform scale-100 transition-all duration-300">
            ...
            <p className="text-gray-600 mb-6 text-sm leading-relaxed">
              This action cannot be undone. You are about to permanently delete <strong>{selectedIds.length} science lead{selectedIds.length > 1 ? 's' : ''}</strong> from the database.
            </p>
...
export default ScienceConsultationList;
