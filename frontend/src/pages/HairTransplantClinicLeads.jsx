...
import { 
   ...
   Trash2, Eye, Download, ChevronLeft, 
   ChevronRight, CalendarDays, Clock, User,
   CheckCircle, PhoneOutgoing, X, Save,
   Check, ChevronDown, CheckSquare, Square, AlertTriangle, Loader2, Inbox
} from "lucide-react";
import Loader from "../components/Loader";
import {
  getHairTransplantLeads,
  updateHairTransplantLead,
  deleteHairTransplantLead,
  bulkDeleteHairTransplantLeads,
  exportHairTransplantLeadsCsv
} from "../api/services";
...
function HairTransplantClinicLeads() {
  ...
  const fetchItems = async (page = pagination.page) => {
    setLoading(true);
    try {
      const response = await getHairTransplantLeads({
        page,
      ...
      toast.error("Unable to load Hair Transplant clinic leads");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id, e) => {
    ...
    try {
      await deleteHairTransplantLead(id);
  ...
  const handleQuickContact = async (id, e) => {
    ...
    try {
      await updateHairTransplantLead(id, { status: "contacted" });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await bulkDeleteHairTransplantLeads({ ids: selectedIds });
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      await exportHairTransplantLeadsCsv({
        search,
        status,
        startDate,
        endDate
  ...
  const handleSaveCRM = async (e) => {
    ...
    setSavingCrm(true);
    try {
      await updateHairTransplantLead(crmModal.item._id, crmForm);
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-[28px] shadow-sm border border-slate-100">
        <div>
          <h3 className="text-2xl font-bold text-slate-900">Hair Transplant Clinic Leads</h3>
          <p className="text-sm text-slate-500 mt-1">Manage consultation requests originating from the redesigned hair transplant page</p>
        </div>
      ...
      {loading && items.length === 0 ? (
        <Loader label="Retrieving Hair Transplant clinic leads..." />
      ) : items.length === 0 ? (
        <div className="bg-white/50 backdrop-blur-md rounded-[28px] border border-slate-100 p-12 text-center max-w-xl mx-auto my-12 shadow-sm">
          ...
          <p className="text-gray-500 text-sm leading-relaxed max-w-xs mx-auto">
            Consultation requests submitted on `/hair-transplant-clinic-in-delhi` will populate here.
          </p>
        </div>
      ) : (
...
export default HairTransplantClinicLeads;
